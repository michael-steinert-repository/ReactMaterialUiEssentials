...
const useTable = (records, headCells, filterFunction) => {
    ...
    const TableHead = (props) => {
        const handleSortRequest = (headCellId) => {
            const isAscending = (orderBy === headCellId && order === "asc");
    ...
    const stableSort = (records, comparator) => {
        const stabilizedThis = records.map((element, index) => [element, index]);
    ...
    const recordsAfterPagingAndSorting = () => {
        return stableSort(filterFunction.filter(records), getComparator(order, orderBy)).slice((page * rowsPerPage), ((page + 1) * rowsPerPage));
    }
